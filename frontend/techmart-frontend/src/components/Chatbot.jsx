      ...
      <div>
          ...
          <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:6 }}>
            {msg.data.items?.map(p => (
              <div key={p._id} onClick={() => navigate(`/product/${p._id}`)}
                style={{ display:"flex", alignItems:"center", gap:8, background:"#111", border:"1px solid #2a2a2a", borderRadius:8, padding:8, cursor:"pointer" }}>
                ...
                <button onClick={e => { e.stopPropagation(); sendMessage(`Add ${p.name} to cart`); }} style={{ background:"#f97316", border:"none", color:"#fff", borderRadius:6, padding:"4px 8px", fontSize:10, cursor:"pointer" }}>+Cart</button>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ marginTop:8, display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
            {msg.data.items?.map(p => (
              <div key={p._id} onClick={() => navigate(`/product/${p._id}`)}
                ...
                <p style={{ color:"#fff", fontSize:11, fontWeight:600, margin:"0 0 2px" }}>{p.name}</p>
        ...
        {msg.data?.type === "balance" && (
          <div style={{ marginTop:8, background:"#0a2a0a", border:"1px solid #22c55e", borderRadius:8, padding:10, textAlign:"center" }}>
            <p style={{ color:"#888", fontSize:11, margin:"0 0 2px" }}>Wallet Balance</p>
          ...
          <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:6 }}>
            {msg.data.items?.map(o => (
              <div key={o._id} onClick={() => navigate("/tracking")}
                style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:8, padding:8, cursor:"pointer" }}>
                <p style={{ color:"#fff", fontSize:12, fontWeight:600, margin:"0 0 2px" }}>#{o.trackingNumber || o._id?.slice(-6)}</p>
                <p style={{ color:"#888", fontSize:11, margin:0 }}>₦{o.amount?.toLocaleString()} • <span style={{ color: o.status==="Delivered"?"#22c55e":o.status==="Cancelled"?"#dc2626":"#f97316" }}>{o.status}</span></p>
              </div>
            ))}
          </div>
        )}
        {msg.data?.type === "coupon" && (
          <div onClick={() => executeAction("apply_coupon", msg.data)}
            ...
            <p style={{ color:"#888", fontSize:11, margin:0 }}>Tap to copy</p>
          </div>
        )}
        {msg.data?.type === "descriptions" && (
          <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:6 }}>
            {msg.data.items?.map((d, i) => (
              <div key={i} style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:8, padding:8 }}>
                <p style={{ color:"#f97316", fontSize:11, fontWeight:700, margin:"0 0 4px" }}>{d.name}</p>
                <p style={{ color:"#aaa", fontSize:11, margin:0 }}>{d.description}</p>
              </div>
            ))}
          </div>
        )}
        {msg.data?.type === "users" && (
          <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:4 }}>
            {msg.data.items?.map((u, i) => (
              <div key={i} style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:8, padding:8 }}>
                <p style={{ color:"#fff", fontSize:12, fontWeight:600, margin:"0 0 2px" }}>{u.name} <span style={{ color:"#888", fontWeight:400 }}>({u.email})</span></p>
        ...
        {pendingAction?.action === "confirm_transfer" && msg.data?.type === "confirm_transfer" && (
          <div style={{ marginTop:8, display:"flex", gap:6 }}>
            <button onClick={() => executeAction("confirm_transfer", pendingAction.data)} style={{ flex:1, padding:8, background:"linear-gradient(135deg,#f97316,#dc2626)", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:12 }}>Confirm</button>
            <button onClick={() => { setPendingAction(null); addMessage("Cancelled.", "bot"); }} style={{ flex:1, padding:8, background:"#222", color:"#888", border:"1px solid #333", borderRadius:8, cursor:"pointer", fontSize:12 }}>Cancel</button>
          </div>
        )}
        {pendingAction?.action === "confirm_split" && msg.data?.type === "confirm_split" && (
          <div style={{ marginTop:8, display:"flex", gap:6 }}>
            <button onClick={() => executeAction("confirm_split", pendingAction.data)} style={{ flex:1, padding:8, background:"linear-gradient(135deg,#f97316,#dc2626)", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:12 }}>Split Now</button>
            <button onClick={() => { setPendingAction(null); addMessage("Cancelled.", "bot"); }} style={{ flex:1, padding:8, background:"#222", color:"#888", border:"1px solid #333", borderRadius:8, cursor:"pointer", fontSize:12 }}>Cancel</button>
          </div>
        )}
        {pendingAction?.action === "confirm_buy" && msg.data?.type === "confirm_buy" && (
          <div style={{ marginTop:8, display:"flex", gap:6 }}>
            <button onClick={() => executeAction("confirm_buy", pendingAction.data)} style={{ flex:1, padding:8, background:"linear-gradient(135deg,#22c55e,#16a34a)", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:12 }}>Buy Now</button>
            <button onClick={() => { setPendingAction(null); addMessage("Cancelled.", "bot"); }} style={{ flex:1, padding:8, background:"#222", color:"#888", border:"1px solid #333", borderRadius:8, cursor:"pointer", fontSize:12 }}>Cancel</button>
          </div>
        )}
      </div>
  ...
  return (
    <>
      ...
      <button onClick={() => setIsOpen(o => !o)}
        style={{ position:"fixed", bottom:"24px", right:"80px", width:"52px", height:"52px", borderRadius:"50%", background:"linear-gradient(135deg,#f97316,#dc2626)", border:"none", fontSize:"22px", cursor:"pointer", boxShadow:"0 4px 20px rgba(249,115,22,0.5)", zIndex:9998, display:"flex", alignItems:"center", justifyContent:"center" }}>
        {isOpen ? "✕" : "🤖"}
        {alerts.length > 0 && !isOpen && <span style={{ position:"absolute", top:-4, right:-4, background:"#dc2626", color:"#fff", borderRadius:"50%", width:16, height:16, fontSize:10, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>{alerts.length}</span>}
      </button>

      {isOpen && (
        <div style={{ position:"fixed", bottom:"90px", right:"80px", width:"380px", height:"560px", background:"#0a0a0a", border:"1px solid #222", borderRadius:"16px", display:"flex", flexDirection:"column", zIndex:9999, boxShadow:"0 20px 60px rgba(0,0,0,0.8)", overflow:"hidden", animation:"fadeIn 0.2s ease" }}>
          {/* HEADER */}
          <div style={{ padding:"14px 16px", background:"linear-gradient(135deg,#f97316,#dc2626)", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <div>
                <p style={{ color:"#fff", fontWeight:800, fontSize:14, margin:0 }}>TechMart AI</p>
                <p style={{ color:"rgba(255,255,255,0.8)", fontSize:11, margin:0 }}>Smart Assistant • {userName} {userRole !== "customer" && `• ${userRole}`}</p>
              </div>
            </div>
            <div style={{ display:"flex", gap:6 }}>
              {speaking && <button onClick={() => window.speechSynthesis?.cancel()} style={{ background:"rgba(0,0,0,0.2)", border:"none", color:"#fff", borderRadius:6, padding:"4px 8px", cursor:"pointer", fontSize:11 }}>Stop</button>}
          ...
          {alerts.length > 0 && (
            <div style={{ background:"#1a0a00", borderBottom:"1px solid #f97316", padding:"6px 12px", display:"flex", alignItems:"center", gap:6 }}>
              ...
              <button onClick={() => setAlerts(a => a.slice(1))} style={{ background:"none", border:"none", color:"#888", cursor:"pointer", fontSize:14 }}>✕</button>
            </div>
          )}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:12, display:"flex", flexDirection:"column", gap:8 }}>
            {messages.map((msg, i) => (
              <div key={msg.id || i} style={{ display:"flex", justifyContent: msg.sender === "user" ? "flex-end" : "flex-start", animation:"fadeIn 0.2s ease" }}>
                <div style={{ maxWidth:"88%", padding:"10px 12px", borderRadius: msg.sender === "user" ? "12px 12px 0 12px" : "12px 12px 12px 0", background: msg.sender === "user" ? "linear-gradient(135deg,#f97316,#dc2626)" : "#1a1a1a", border: msg.sender === "bot" ? "1px solid #222" : "none", color:"#fff", fontSize:13, lineHeight:1.5 }}>
                  {renderMsg(msg)}
            ...
            {typing && (
              <div style={{ display:"flex", justifyContent:"flex-start" }}>
                <div style={{ background:"#1a1a1a", border:"1px solid #222", borderRadius:12, padding:"10px 14px", display:"flex", gap:4, alignItems:"center" }}>
          ...
          <div style={{ padding:"10px 12px", borderTop:"1px solid #1a1a1a", display:"flex", gap:8, alignItems:"center", flexShrink:0 }}>
            <button onClick={startVoice}
              style={{ width:36, height:36, borderRadius:"50%", background: listening ? "#dc2626" : "#1a1a1a", border:"1px solid #333", color: listening ? "#fff" : "#888", cursor:"pointer", fontSize:16, flexShrink:0, animation: listening ? "pulse 1s infinite" : "none" }}>
              🎤
            </button>
            ...
            <button onClick={() => sendMessage()}
              style={{ width:36, height:36, borderRadius:"50%", background:"linear-gradient(135deg,#f97316,#dc2626)", border:"none", color:"#fff", cursor:"pointer", fontSize:16, flexShrink:0 }}>
              ➤
            </button>
          </div>
        </div>
